...
export function HippoProvider({ children }: { children: React.ReactNode }) {
    ...
    const updateStats = useCallback((newStats: Partial<HippoStats>) => {
        setHippo(prev => {
            ...
            storage.setItem('hippoStats', JSON.stringify(updatedStats)).catch(
                (error: unknown) => console.error('Failed to save stats:', error)
            );
            return updatedHippo;
        });
    }, []);
    ...
    const feed = useCallback(() => {
        ...
        setHippo(prev => {
            ...
            storage.setItem('hippoFeedCount', updated.feedCount.toString())
                .catch((error: unknown) => console.error('Failed to save feed count:', error));
            return updated;
        });
    ...
    const clean = useCallback(() => {
        ...
        setHippo(prev => {
            ...
            storage.setItem('hippoCleanCount', updated.cleanCount.toString())
                .catch((error: unknown) => console.error('Failed to save clean count:', error));
            return updated;
        });
    ...
    const addCoins = useCallback((amount: number) => {
        setHippo(prev => {
            ...
            storage.setItem('hippoCoins', updated.coins.toString()).catch(
                (error: unknown) => console.error('Failed to save coins:', error)
            );
            return updated;
        });
    }, []);
    ...
    const play = useCallback((): boolean => {
        ...
        setHippo(prev => {
            ...
            storage.setItem('hippoPlayCount', updated.playCount.toString())
                .catch((error: unknown) => console.error('Failed to save play count:', error));
            return updated;
        });
    ...
    const updateGameStats = useCallback((gameType: 'bubble' | 'memory' | 'third', score: number = 0) => {
        setHippo(prev => {
            ...
            storage.setItem('hippoGameStats', JSON.stringify(updatedGameStats)).catch(
                (error: unknown) => console.error('Failed to save game stats:', error)
            );
    ...
    const sleep = useCallback(() => {
        ...
        setHippo(prev => {
            ...
            storage.setItem('hippoSleepCount', updated.sleepCount.toString())
                .catch((error: unknown) => console.error('Failed to save sleep count:', error));
            return updated;
        });
    ...
    const giveWater = useCallback(() => {
        ...
        setHippo(prev => {
            ...
            storage.setItem('hippoWaterCount', updated.waterCount.toString())
                .catch((error: unknown) => console.error('Failed to save water count:', error));
            return updated;
        });
        ...
        if (hippo.coins >= item.price) {
            setHippo(prev => {
                ...
                storage.setItem('hippoOutfit', JSON.stringify(updatedOutfit)).catch(
                    (error: unknown) => console.error('Failed to save outfit:', error)
                );
            ...
            storage.setItem('unlockedItems', JSON.stringify(Array.from(newUnlockedItems))).catch(
                (error: unknown) => console.error('Failed to save unlocked items:', error)
            );
    ...
    const setHippoAsync = useCallback((newHippo: Hippo) => {
        ...
        Promise.all(savePromises).catch((error: unknown) => {
